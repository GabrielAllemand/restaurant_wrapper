...
BASE_TABLE = "establishments"
PARIS_ENRICHED_VIEW = "vw_establishments_paris_enriched"
...
def _source_name(only_paris: bool = False) -> str:
    return PARIS_ENRICHED_VIEW if only_paris else BASE_TABLE


def _phone_select_expr(only_paris: bool = False) -> str:
    return "phone_final AS phone" if only_paris else "phone"


def _rating_select_expr(only_paris: bool = False) -> str:
    return "rating_final AS rating" if only_paris else "rating"


def _review_count_select_expr(only_paris: bool = False) -> str:
    return "review_count_final AS review_count" if only_paris else "review_count"


def _extra_scraping_select_sql(only_paris: bool = False) -> str:
    if not only_paris:
        return """
        NULL AS phone_base,
        NULL AS phone_scraped,
        NULL AS rating_base,
        NULL AS rating_scraped,
        NULL AS review_count_base,
        NULL AS review_count_scraped,
        NULL AS scrape_review_platform,
        NULL AS scrape_description,
        NULL AS scrape_tags,
        NULL AS scrape_type,
        NULL AS scrape_status,
        NULL AS scrape_source,
        NULL AS scrape_source_url,
        NULL AS scrape_match_mode,
        NULL AS scrape_match_confidence,
        NULL AS scrape_name_score_source,
        NULL AS scrape_name_score_inpi,
        NULL AS scrape_best_name_score,
        NULL AS scrape_address_score,
        NULL AS scrape_phone_match,
        NULL AS scrape_same_postal_code,
        NULL AS scrape_final_score,
        NULL AS scrape_candidate_count,
        NULL AS scrape_score_gap_vs_second,
        NULL AS scrape_loaded_at,
        """
    ...
    scrape_source_url,
    scrape_match_mode,
    scrape_match_confidence,
    scrape_name_score_source,
    scrape_name_score_inpi,
    scrape_best_name_score,
    scrape_address_score,
    scrape_phone_match,
    scrape_same_postal_code,
    scrape_final_score,
    scrape_candidate_count,
    scrape_score_gap_vs_second,
    scrape_loaded_at,
    """
...
def load_full_dataset(limit: int | None = None, only_paris: bool = False) -> pd.DataFrame:
    engine = get_engine()
    source_name = _source_name(only_paris=only_paris)
    ...
    SELECT
        ...
        latitude,
        longitude,
        {_phone_select_expr(only_paris=only_paris)},
        email,
        website,
        opening_hours,
        cuisine,
        {_rating_select_expr(only_paris=only_paris)},
        {_review_count_select_expr(only_paris=only_paris)},
        {_extra_scraping_select_sql(only_paris=only_paris)}
        siret,
        business_status,
        ban_address,
        ban_city,
...
@st.cache_data(ttl=600, show_spinner=False)
def get_categories(only_paris: bool = False) -> list[str]:
    engine = get_engine()
    source_name = _source_name(only_paris=only_paris)
    sql = f"""
...
@st.cache_data(ttl=600, show_spinner=False)
def get_names_for_search(limit: int = 5000, only_paris: bool = False) -> list[str]:
    engine = get_engine()
    source_name = _source_name(only_paris=only_paris)
    sql = f"""
    SELECT name
...
def search_entities(
    ...
    phone_field = "phone_final" if only_paris else "phone"
    rating_field = "rating_final" if only_paris else "rating"
    review_count_field = "review_count_final" if only_paris else "review_count"
    ...
    if has_phone is True:
        conditions.append(f"{phone_field} IS NOT NULL")
    elif has_phone is False:
        conditions.append(f"{phone_field} IS NULL")
    ...
    SELECT
        ...
        email,
        website,
        {rating_field} AS rating,
        {review_count_field} AS review_count,
        {_extra_scraping_select_sql(only_paris=only_paris)}
        siret,
        collected_at,
        ban_address,
...
@st.cache_data(ttl=600, show_spinner=False)
def get_global_kpis(only_paris: bool = False) -> pd.DataFrame:
    engine = get_engine()
    source_name = _source_name(only_paris=only_paris)
    phone_field = "phone_final" if only_paris else "phone"
    rating_field = "rating_final" if only_paris else "rating"
    ...
    SELECT
        ...
        COUNT({phone_field}) AS phone_filled,
        COUNT(website) AS website_filled,
        COUNT(email) AS email_filled,
        COUNT({rating_field}) AS rating_filled,
        COUNT({review_count_field}) AS review_count_filled,
...
def get_market_table(
    ...
    SELECT
        {geo_expr} AS geography,
        COUNT(*) AS total_rows,
        ROUND(100.0 * COUNT({phone_field}) / COUNT(*), 2) AS pct_phone,
